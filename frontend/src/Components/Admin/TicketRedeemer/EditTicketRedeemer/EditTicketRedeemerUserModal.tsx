...
import { EditTicketRedeemerUserModalSkeleton } from "./EditTicketRedeemerModalSkeleton";

// import { getUser, updateUser } from "@/lib/frontend/http_calls/admin";

import { useAsyncEffect } from "use-async-effect";
import "./edit.ticket.redeemer.css";
import {
  getTicketRedeemerUser,
  resetPassword,
  updateTicketRedeemer,
...
export const EditTicketRedeemerUserModal = (props: Props) => {
  ...
  async function populateInitialData() {
    let response = await getTicketRedeemerUser(props.id);
    if (response.error) {
  ...
  async function onFinish(data: any) {
    setIsLoading(true);
    setShow(false);
    let response = await updateTicketRedeemer(props.id, data);
    if (response.error) {
  ...
  return (
    <div className="customModalCtx">
      ...
      >
        {isModalLoading ? (
          <EditTicketRedeemerUserModalSkeleton />
        ) : (
          <>
